...
stmnt returns [List<Statement> ast = new ArrayList<>();]:
		...
		| unaryIncDec ';' {$ast.add($unaryIncDec.ast);}
		;
		
unaryIncDec returns [UnaryIncrementDecrement ast]:
			ID '++'	{$ast = new UnaryIncrementDecrement($start.getLine(),$start.getCharPositionInLine()+1,new Variable($ID.getLine(),$ID.getCharPositionInLine()+1,$ID.getText()),"++",false);}
		|	ID '--'	{$ast = new UnaryIncrementDecrement($start.getLine(),$start.getCharPositionInLine()+1,new Variable($ID.getLine(),$ID.getCharPositionInLine()+1,$ID.getText()),"--",false);}
		|	'++' ID	{$ast = new UnaryIncrementDecrement($start.getLine(),$start.getCharPositionInLine()+1,new Variable($ID.getLine(),$ID.getCharPositionInLine()+1,$ID.getText()),"++",true);}
		|	'--' ID	{$ast = new UnaryIncrementDecrement($start.getLine(),$start.getCharPositionInLine()+1,new Variable($ID.getLine(),$ID.getCharPositionInLine()+1,$ID.getText()),"--",true);}
	;
...
exp returns [Expression ast]:
	...
	| unaryIncDec {$ast = $unaryIncDec.ast;}
	;
